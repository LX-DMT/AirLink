...
int aicwf_sdio_func_init(struct aic_sdio_dev *sdiodev)
{
	...
	u8 cccr_speed = 0;
	unsigned int original_clock;
	unsigned int original_timing;
	int ret = 0;
	int restore_ret = 0;
	bool high_speed_switched = false;
	...
	original_clock = host->ios.clock;
	original_timing = host->ios.timing;
	...
	if (feature.sdio_clock > 0) {
		pr_info("aicbsp: SDIO clock transition: requested=%u actual=%u timing=%u caps=0x%x\n",
			  feature.sdio_clock, host->actual_clock,
			  host->ios.timing, host->caps);

		if (feature.sdio_clock > 25000000) {
			if (!(host->caps & MMC_CAP_SD_HIGHSPEED)) {
				sdio_err("host does not advertise SD high-speed capability\n");
				ret = -EOPNOTSUPP;
				goto release_host;
			}

			sdiodev->func->card->quirks |= MMC_QUIRK_LENIENT_FN0;
			cccr_speed = sdio_f0_readb(sdiodev->func,
						 SDIO_CCCR_SPEED, &ret);
			if (ret) {
				sdio_err("read CCCR SPEED failed: %d\n", ret);
				goto release_host;
			}
			if (!(cccr_speed & SDIO_SPEED_SHS)) {
				sdio_err("card does not advertise SDIO high-speed support: CCCR_SPEED=0x%02x\n",
					 cccr_speed);
				ret = -EOPNOTSUPP;
				goto release_host;
			}

			sdio_f0_writeb(sdiodev->func,
				       cccr_speed | SDIO_SPEED_EHS,
				       SDIO_CCCR_SPEED, &ret);
			if (ret) {
				sdio_err("enable CCCR EHS failed: %d\n", ret);
				goto release_host;
			}
			if (!(sdio_f0_readb(sdiodev->func, SDIO_CCCR_SPEED,
					    &ret) & SDIO_SPEED_EHS) || ret) {
				sdio_err("CCCR EHS readback failed: %d\n", ret);
				sdio_f0_writeb(sdiodev->func, cccr_speed,
					       SDIO_CCCR_SPEED, &restore_ret);
				ret = ret ? ret : -EIO;
				goto release_host;
			}
			...
			high_speed_switched = true;
		}

		host->ios.clock = feature.sdio_clock;
		host->ops->set_ios(host, &host->ios);
		udelay(10);
		pr_info("aicbsp: SDIO clock transition done: requested=%u actual=%u timing=%u cccr_speed=0x%02x\n",
			  host->ios.clock, host->actual_clock,
			  host->ios.timing, cccr_speed | SDIO_SPEED_EHS);
	}
#endif
...
restore_bus:
	if (high_speed_switched) {
		sdio_claim_host(sdiodev->func);
		host->ios.clock = original_clock;
		host->ios.timing = original_timing;
		host->ops->set_ios(host, &host->ios);
		udelay(10);
		sdio_f0_writeb(sdiodev->func, cccr_speed,
			       SDIO_CCCR_SPEED, &restore_ret);
		sdio_err("SDIO transition failed, restored clock=%u actual=%u timing=%u CCCR ret=%d\n",
			 host->ios.clock, host->actual_clock,
			 host->ios.timing, restore_ret);
		sdio_release_host(sdiodev->func);
	}
	return ret;
}
