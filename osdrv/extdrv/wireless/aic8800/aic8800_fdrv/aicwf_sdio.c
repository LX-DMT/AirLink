...
int aicwf_sdio_func_init(struct aic_sdio_dev *sdiodev)
{
	...
	if (feature.sdio_clock > 0) {
		AICWFDBG(LOGINFO, "SDIO before clock switch: requested=%u actual=%u timing=%u bus_width=%u power_mode=%u\n", host->ios.clock, host->actual_clock, host->ios.timing, host->ios.bus_width, host->ios.power_mode);
		host->ios.clock = feature.sdio_clock;
		host->ops->set_ios(host, &host->ios);
		AICWFDBG(LOGINFO, "Set SDIO Clock %d MHz\n", host->ios.clock/1000000);
		AICWFDBG(LOGINFO, "SDIO after clock switch: requested=%u actual=%u timing=%u bus_width=%u power_mode=%u\n", host->ios.clock, host->actual_clock, host->ios.timing, host->ios.bus_width, host->ios.power_mode);
	}
#endif
